...
\begin{equation}
    ...
    \displaystyle
        ...
        \left(0,\ 1,\ 1/c_s
        ,\ 1/c_s^2 \rigth)\ e^{-g\ z/cs^2}
\end{equation}
This solution is a classical Lamb waves propagating in a semi-infinite medium (such as the atmosphere). To propagate in the ocean, it must also satisfy the surface kinematic relation obtained by retaining second-order terms at the (otherwise-vanishing) right-hand-side of \ref{WM_bc_b}. To do so, a Taylor expansion of the pressure field in the vicinity of the surface brings to:
\begin{equation}
    \displaystyle
...
\begin{equation}
    ...
    -\zeta\ \frac{\partial ^2 p}{\partial z ^2}(z=0) 
    + \mathrm{O}(\zeta ^2)\\[4mm]
    &=&
    ...
    \end{array}
\end{equation}
The computation of the surface elevation anomaly $\zeta$ from this latter relation requires the use of an extended system of equations (including as a variable the velocity and not the momentum) and is, consequently, out of the scope of the present study. In the frame of the model presented here, Lamb waves can be included under the (first-order) assumption of rigid-lid (i.e. $\partial\zeta / \partial t \approx 0$).\\
...
\begin{subequations}
    ...
    F''(z)
    +
   \overbrace{ (k_x^2\frac{N_c^2-\Omega^2}{\Omega^2}+\frac{\Omega^2}{c_s^2}-\frac{1}{4D_0^2}\right)}^{\equiv k_z^2}
    F(z)=0\\[4mm]
	\label{EqDimFb}
    \displaystyle
    F'(0)+\left(
...
The usual dispersion relation for (very) long surface waves with well-known $\sqrt{g H}$ phase-velocity (i.e. waves satisfying $\delta_z=\delta_{z_vlsw}=\delta_x$ and $\omega=\omega_{vlsw}=\delta_x$ in an incompressible, inhomogeneous ocean) can be recovered from the swell-like relation. \\
...
\begin{equation}
	...
	3(2+\epsilon_i^2+\epsilon_a^2)
	(4\epsilon_i^2+\epsilon_i^4-\epsilon_a^4)
	}
	{
	12(2+\epsilon_i^2-\epsilon_a^2)
	}
\end{equation}
...
Waves propagating with vanishing vertical wave-number ($\delta_z=0$) have already been encountered in subsection \ref{SubSectionDisp} where they have been shown to satisfy the dispersion relations \ref{EqDispInnerkz0} and \ref{EqDispBoundkz0}. In non-dimensional form, these relations can be rewritten:
\begin{subequations}
...
In a stratified ocean (whether compressible or not), the lower branch of the inner dispersion surface does not lead to physical solutions with positive, real horizontal vertical wave-numbers (figure \oldref{FigFullHomogeneous}.d).
Surface waves can propagate but as long (mode-0) MIM solution. 
...
\color{red} Comments needed here for LMSW to link $\delta_x\approx\delta_z$ and $\delta_z=0$ cases. \color{black}\\

%The \textit{barotropic mode} $(n=0)$ disappears and the inner dispersion surface does not intersect the boundary dispersion surface in the vicinity of the origin: the horizontal and vertical wave-numbers of the barotropic MSW must simultaneously vanish. In an homogeneous ocean, the evanescent surface wave solution MSW is valid till the origin where $\delta_z$, $\delta_x$ and $\omega$ all vanish. \\
...
In the long-wave limit now, a modified barotropic mode (MIM-0) exists only in a stratified ocean. It can be associated to a low-frequency oscillation of the stratified water layer. It is part of the small-pulsation branch of the real-$\delta_z$ inner dispersion surface. It thus cannot be the asymptotic limit of the shorter swell-like MSW branch. Usual approximations for long surface waves ($\delta_z=\delta_x$ and $\omega=\delta_x$) can be recovered from the results of the present studies in two ways. It can either be introduced as a long-wave approximation of MSW for homogeneous (non-stratified) ocean or as a long-wave approximation of mode-0 MIM. In this latter case, a vanishing vertical wave-number is recovered only for $\epsilon_i=\epsilon_a=0$.
