...
\begin{table}[!h]
	...
	\begin{tabular} {c|c|c|c}
		...
		MAW & Modified Acoustic Wave & \ref{DispAcousDT} & \ref{DispAcousDT}\\
		MIW & Modified Internal Wave ray & \ref{DispRaysDT} & \ref{DispRaysDT}\\\hline
		MAM & Modified Acoustic Mode & \ref{DispSysAcousModes12} & LMAM (\oldref{ParamLMAM1}, \oldref{ParamLMAM2}) SMAM (\oldref{ParamSMAM1}, \oldref{ParamSMAM2})\\
		MSW & Modified Surface Wave & \ref{EqFullDisperai} \& \ref{EqFullDisperbi} & \ref{EqDispLonga1} \& \ref{Eqdzdx2} \\
		LMSW & Long Modified Surface Wave & \ref{EqFullDisperai} \& \ref{EqFullDisperbi} & \ref{Eqxxx} \& \ref{EqDispDeltaz0} \\
		MIM & Modified Internal Mode 1 & \ref{DispSysIntModesab} & LMIM (\oldref{ParamMIM1}, \oldref{ParamMIM2})\\
		LMIM0 & Long Gravity Oscillation (LMIM 0)& \ref{EqFullDispera} \& \ref{Eqxxx} & LMIM (\oldref{MIMomega2}, \oldref{MIMdz2})\\
	\end{tabular}
...
Dukowicz (2013) tackled with this problem and proposed a review of the \textit{"various approximations in atmosphere and ocean model based on an exact treatment of gravity wave dispersion"}. In the ocean, acoustic-gravity waves are shown by the author to satisfy a system of two dispersion relations and the impact of several usual assumptions often made in ocean models is evaluated. The present study builds on Dukowicz's study and specifically focuses on the consequences of both the stratification and compressibility of the ocean on the properties of acoustic-gravity waves and wave-modes: Taylor expansions of dispersion relations and of the many resulting expressions for wavelengths and pulsation are derived in terms of both compressibility and stratification. In addition:
\begin{itemize}
	\item We show that free-surface boundary constraints can be recovered at the same order of precision as in Dukowicz's but using an Eulerian framework. We do not intent to show that the Eulerian framework makes more physical sens than the Lagragian framework but just that it can reduce the analytical burden.
	...
	\item A particular region of phase space is identified where surface acoustic-gravity surface wave are "structurally singular" in the sens that gravity and compressiblity are both important and the corresponding wave solution is close to instability in time.
\end{itemize}   
...
\begin{subequations}
  ...
  & &&= \underbrace{
  \underbrace{\displaystyle \hat{\rho}_h(z) g (\zeta-z)}_{\hat{p}_h(z)}
...
\begin{subequations}
 ...
 -\overbrace{\frac{c_s^2 N_c^2}{g}}^{c_s^2 \frac{N^2}{g}-g}\hat{\rho}_hw
  \end{alignat}
\end{subequations}
...
\begin{subequations}
    ...
    F''(z)
    +
   \overbrace{ \left(k_x^2\frac{N^2-\Omega^2}{\Omega^2}+\frac{\Omega^2}{c_s^2}-\frac{1}{4D_0^2}\right)}^{\equiv k_z^2}
    F(z)=0\\[4mm]
	\label{EqDimFb}
    \displaystyle
    F'(0)+\left(
...
\begin{equation}
  ...
  \displaystyle
   k_x^2\left(1-\frac{N^2}{\Omega^2}\right)+k_z^2
...
\begin{subequations}
	...
	&&=\frac{\epsilon_i^4\epsilon_a^2\delta_x^2}{(\delta_x^2+\delta_z^2)^3}
...
\begin{equation}
	...
	\frac{1}{4}
\end{equation}
Both roots are real and distinct whatever $\delta_x$ and $\delta_z$. \\
...
\begin{subequations}
	...
	+(\epsilon_a^2+\epsilon_i^2)^2/4}
	&&\ \leq\  
	\omega_{-}^2(\delta_x,\ i\delta_{z,i})=
	\frac{\delta_x^2\ \epsilon_i^2}{\delta_x^2-\delta_{z,i}^2
	+(\epsilon_a^2+\epsilon_i^2)^2/4}\\[3mm]
	\nonumber& &&\ \leq\  
	\omega_{+}^2(\delta_x,\ i\delta_{z,i})=\frac{1}{\epsilon_a^2}\left(
...
Far from this region, i.e. if $\exists\ \alpha>0\ /\ R^2\leq 1/4(1-\alpha^2)$ or in $(\delta_x,\ \delta_z,\ \omega)$ phase-space (sufficient relation):
\begin{equation}
	\label{regsepi}
...
\begin{subequations}
	...
	\end{alignat}
\end{subequations}
and Relations \ref{Comp1} and \ref{Comp2} show that:
\begin{subequations}
	\begin{alignat}{2}
	\label{Comp1b}
	&\omega_a^2(\delta_x,\ \delta_z)-\omega_{+}^2(\delta_x,\ \delta_z)
	...
	\end{alignat}	
\end{subequations}
These relations show that for $\delta_z\in i\ \mathbb{R}$, the roots are well-separated in the region of phase-space far enough from the plane $\delta_x=\delta_{z,i}$ \ref{regsepi} and, in such a region, the roots can be as close as required from the acoustic (stratification) functions if $\delta_x$ is decreased (respectively increased).
...
When $\delta_z\in \mathbb{R}$, a second-order Taylor development of the gravity-wave solution $\omega_{-}$ \ref{SolGrav} with respect to small parameters $\epsilon_a$ and $\epsilon_i$ leads to:

\begin{equation}
	\label{DispRaysDT}
		\frac{\omega^2}{\epsilon_i^2} =\underbrace{
		\frac{\delta_x^2}{\delta_x^2+\delta_z^2}
		\left(1-\frac{\epsilon_i^2+\epsilon_a^2}{4(\delta_x^2+\delta_z^2)}\right)}_{\approx\ \omega_i^2/\epsilon_i^2}
		+\frac{\epsilon_i^2\epsilon_a^2\delta_x^2}{(\delta_x^2+\delta_z^2)^3}
...
\begin{equation}
	...
	\label{DispRays}
\end{equation}
is recovered at second order in $\epsilon_i$ and $\epsilon_a$. Relations \ref{DispRaysDT} and \ref{DispRays} further show that $\omega_{iwr}^2/\epsilon_i^2$ is a second-order approximation of $\omega_i^2/\epsilon_i^2$ and is itself a fourth-order approximation of the general solution $\omega^2/\epsilon_i^2$.
As a consequence, when parameters $\epsilon_i^2$ and $\epsilon_a^2$  are both small, the stratification function $\omega_i(\delta_x,\delta_z)$ is an internal gravity wave ray modified by compressibility and propagating in an unbounded ocean. The first term of the second-order correction is proportional to $\epsilon_i^2+\epsilon_a^2=H/D_0$ and consequently depends on the relative strength of the stratification (with no compressibility correction). It can only reduces the pulsation. The second term depends on the product of $\epsilon_i^2$ and $\epsilon_a^2$ and thus both on the stratification and compressibility. It increases the pulsation.\\
...
\begin{figure}[!h]
	...
	\begin{subfigure}{0.45\linewidth}
		\includegraphics[width=1\linewidth]
		{FIGURES/Fig_App3_Real_Grav.png}
		\caption{}
	\end{subfigure}
...
MIM (red \& pink): numerical approximation (red, solid), long MIM for  $n=1$ $(\delta_{z,lmim1},\ \omega_{lmim1})$ (red, dash-dotted), MIM for $n=0$ $(\delta_{z,mim},\ \omega_{mim})$ (pink, solid), long MIM for $n=0$ $(\delta_{z,lmimo},\ \omega_{lmimo})$ (pink, dotted).\\
...
\begin{equation}
	\label{Eqdzdx1}
	\displaystyle
	\delta_z(\delta_x) = \delta_{z,sw}(\delta_x)=\delta_x + O(\epsilon_i^2,\ \epsilon_a^2)
\end{equation}
This crude (but usual) approximation can be substituted in the boundary dispersion relation \ref{EqFullDisperbi}, leading at lowest order in $(\epsilon_i,\epsilon_a)$:
\begin{equation}
	\label{EqDispLonga1}
	\displaystyle
...
\subsubsection{long surface waves ($\delta_z<<1$)}

\textit{Long Modified Surface Waves (MSW):}\\
...
\begin{equation}
	\label{Eqxxx}
     \omega^2=\omega^2_{lmsw}= \delta_x^2 \left( 
 		1+\frac{\delta_{z,i}^2}{3} (1 -\epsilon_i^2-\epsilon_a^2)
 		-\frac{\epsilon_i^2+\epsilon_a^2}{2} \right)
...
\begin{subequations}
	\begin{alignat}{2}
 		& \delta_{z,i}^2 &&=\delta_{z,lmsw}^2(\delta_x)\\[3mm]
		\label{EqDispDeltaz0}
 		& &&=\frac{3}{4}
 		\frac{
...
\end{equation}
giving a lower bound for the surface wave solution. We note that no Taylor development has been carried out either with respect to $\epsilon_i$ and $\epsilon_a$ to provide an accurate description of the MSW. A parametric description for long MSW of the form $(\ \delta_z(\delta_x),\ \omega(\delta_x)\ )$ can be obtained by substituting \ref{EqDispDeltaz0} in \ref{Eqxxx}.\\

\textit{Long Modifed Internal Modes (MIM):}\\
...
\begin{subequations}
	\begin{alignat}{2}
	\label{MIMomega2}
     	& \omega_{lmim}^2(\delta_x,\ \delta_z) &&= \delta_x^2 \left( 
 		1-\frac{\delta_{z}^2}{3} (1 -\epsilon_i^2-\epsilon_a^2)
 		-\frac{\epsilon_i^2+\epsilon_a^2}{2} \right)
 		+ \mathrm{O}(\delta_{z,i}^4)\\[3mm]
	\label{MIMdz2}
 	 &\delta_{z,lmim}^2(\delta_x) &&= -\ \delta_{z,lmsw}^2(\delta_x)
 	\end{alignat}
\end{subequations}
...
A simpler expression can be derived by expending the $(\ \delta_{z,lmim}(\delta_x),\ \omega_{lmim}(\delta_x)\ )$ parametrization with respect to $\delta_x$ and $\epsilon_i\ -\ \epsilon_a$. This leads to a long-wave approximation of MIM of the form:
\begin{subequations}
	\begin{alignat}{2}
	\label{MIMomega}
...
\begin{equation}
	   \label{DispSysIntModesab}
		\omega^2 \approx\omega_i^2=\frac{\epsilon_i^2 \delta_x^2}{\delta_x^2
...
A Taylor development of relation \ref{DispSysIntModesab} with respect to $\delta_z$ is required in the vicinity of $\delta_{z,n}=n\pi$ to obtain an analytical relation of the form $\delta_z(\delta_x)$:
 \begin{equation}
	\delta_{z,n}=\delta_{z,n}^0\left(
	1+\frac{\epsilon_i^2}{\delta_x^2+\delta_{z,n}^2}
	%+\frac{(\delta_x^2-\delta_{z,n}^2)}{(\delta_x^2+\delta_{z,n}^2)^3}\epsilon_i^4
	\right)
	+\mathrm{O}	(\epsilon_i^4,\epsilon_a^4)
\end{equation}
The pulsation $\omega$ can finally be expressed as a function of $\delta_x$ using the expression of the stratification function $\omega_i$:
\begin{equation}
	\omega^2=\epsilon_i^2\frac{\delta_x^2}{\delta_x^2+\delta_{z,n}^2}
	\left(1-\frac{2\epsilon_i^2\delta_{z,n}^2}{(\delta_x^2+\delta_{z,n}^2)^2}\right)
	+\mathrm{O}	(\epsilon_i^6,\epsilon_a^4)
\end{equation}
At order 0 in $\epsilon_a$ and $\epsilon_i$, is recovered the expression of internal modes as expressed in many textbooks:
\begin{subequations}
	\begin{alignat}{2}	
	&\delta_{z,n} &&=\delta_{z,n}^0\\
	&\omega^2 &&=\epsilon_i^2\frac{\delta_x^2}{\delta_x^2+\delta_{z,n}^2}
	\end{alignat}
\end{subequations}
leading to the conclusion that the internal gravity modes are robust to compressibility since there is dependency to $\epsilon_a$ at orders lower than 4 vanishes. This agrees with the separation of the roots $\omega_\pm$ of the inner dispersion relation for real vertical wave-numbers.\\
...
This approximate solution is plotted in figure (\oldref{Fig_Approx}.b) (red dash-dotted line) together with a sixth-order approximation in $\delta_x$ (red dashed line).
...
In Section \ref{SectionGraphic}, the inner and boundary dispersion surfaces have been shown (numerically) to intersect (blue line in figure \oldref{FigDispSolutions}) leading to a Modified Acoustic Mode (MAM). This solution is now derived analytically and its characteristics further investigated.
The root that corresponds to acoustic waves can advantageously be approximated by isolating $\delta_x^2$  in the inner and boundary dispersion relations:
\begin{equation}
	   \label{DispSysAcousModes12}
		\delta_x^2 = \frac{\omega^2}
		{\epsilon_i^2-\omega^2}
		\left(\frac{(\epsilon_i^2+\epsilon_a^2)^2}{4}-\omega^2\right)
		=\omega^2\left(\frac{\epsilon_i^2+\epsilon_a^2}{2}+\delta_z cot(\delta_z)\right)
\end{equation}

\textit{Smith-like MAM (large pulsations)}\\
In a similar way as in the previous subsection, Relation \ref{DispSysAcousModes12} is transcendental in $\delta_z$ but its Taylor expansion successively (i) for $\delta_{z,m} \approx \pi/2+m\pi$ , (ii) for large $\omega$ and (iii) for small parameters $(\epsilon_i^2,\ \epsilon_a^2)$ can be written as an analytical expression of the form $\delta_z(\omega)$:
\begin{equation}
	\delta_z = \delta_{z,m}  \left(1+\frac{1}{\omega^2}-\frac{\epsilon_a^2}{2\delta_{z,m}^2}\right)
	+\mathrm{O}	(\epsilon_i^4,\epsilon_a^4)
\end{equation}
This relation agrees with the acoustic-gravity modes derived by Smith (2013) (first two terms). It additionally shows that these modes are not modified bellow order 4 in $\epsilon_i$ by the stratification and gives the second order dependency in $\epsilon_a$.\\
The square of the horizontal wave-number $\delta_x^2$ can further be expressed as a function of the pulsation $\omega$:
\begin{equation}
	\delta_x^2 = \epsilon_a^2\omega^2
	-\delta_{z,m}^2\left(1+\frac{2}{\omega^2}\right)
	+\epsilon_a^2\left(1+\frac{1}{\omega^2}\right)
	-\epsilon_i^2\left(1+\frac{1+\delta_{z,m}^2}{\omega^2}\right)
	+\mathrm{O}	(\epsilon_i^4,\epsilon_a^4)
\end{equation}
...
Like modified surface and internal gravity waves, modified acoustic waves can be parametrized. Two regions, and thus two physical behaviours, must yet be distinguished. In the vicinity of the origin first $\delta_z$ and $\omega$ can be expressed as functions of $\delta_x$ and of the parameters $(\epsilon_i,\ \epsilon_a)$:
\begin{subequations}
