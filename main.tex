...
\begin{table}[!h]
	...
	\begin{tabular} {c|c|c|c}
		...
		MAM & Modified Acoustic Mode $(m=1)$& \ref{DispSysAcousModes12} & LMAM (\oldref{ParamLMAM1}, \oldref{ParamLMAM2}),\\
		...
		MIM & Modified Internal Mode $(n=1)$  & \ref{DispSysIntModesab} & MIM (\oldref{ParamallMIM1}, \oldref{ParamallMIM2}),\\
...
\textit{Long Modifed Internal Modes (LMIM):}\\
...
\begin{subequations}
	\begin{alignat}{2}
	\label{MIMomega2}
     	& \omega_{lmim}^2(\delta_x,\ \delta_z) &&= \frac{2}{3}\delta_x^2 
...
A Taylor development of relation \ref{DispSysIntModesab} with respect to $\delta_z$ is required in the vicinity of $\delta_{z,n}^0=n\pi$ to obtain an analytical relation of the form $\delta_z(\delta_x)$:
 \begin{equation}
	\label{ParamallMIM1}
	\delta_{z}(\delta_x)=\delta_{z,mim}(\delta_x)=\delta_{z,n}^0\left(
	1+\frac{\epsilon_i^2}{\delta_x^2+\delta_{z,n}^2}
	%+\frac{(\delta_x^2-\delta_{z,n}^2)}{(\delta_x^2+\delta_{z,n}^2)^3}\epsilon_i^4
	\right)
	+\mathrm{O}	(\epsilon_i^4,\epsilon_a^4)
\end{equation}
The pulsation can finally be expressed as a function of $\delta_x$ using the expression of the stratification function $\omega_i$:
\begin{equation}
	\label{ParamallMIM2}
	\omega^2(\delta_x)=\omega_{mim}^2(\delta_x)=\epsilon_i^2\frac{\delta_x^2}{\delta_x^2+\delta_{z,n}^2}
	\left(1-\frac{2\epsilon_i^2\delta_{z,n}^2}{(\delta_x^2+\delta_{z,n}^2)^2}\right)
	+\mathrm{O}	(\epsilon_i^6,\epsilon_a^4)
\end{equation}
	&\omega^2(\delta_x) &&=\omega^2_{im}(\delta_x) =\epsilon_i^2\frac{\delta_x^2}{\delta_x^2+\delta_{z,n}^2}
\begin{subequations}
	\begin{alignat}{2}	
	&\delta_{z,im} &&=\delta_{z,n}^0\\
	&\omega^2(\delta_x) &&=\omega^2_{im}(\delta_x) =\epsilon_i^2\frac{\delta_x^2}{\delta_x^2+\delta_{z,n}^2}
	\end{alignat}
\end{subequations}
...
In Section \ref{SectionGraphic}, a numerical approximation of the intersection of the inner and boundary dispersion surfaces has been computed and plotted (blue line in figure \oldref{FigDispSolutions}). This intersection has been shown to correspond to Modified Acoustic Mode (MAM). MAM is now derived analytically and its characteristics are further investigated. An iterative method is chosen. A first approximation of $\delta_x^2(\omega)$ by crudely assuming $\delta_z(\omega)\approx\delta_{z,m} = \pi/2+m\pi$. Then $\delta_x^2( \omega)$ is substituted in a second-order Taylor expansion of the boundary dispersion relation \ref{EqFullDisperb} to obtain a more accurate approximation of $\delta_z(\omega)$. Its Taylor expansion (i) for large $\omega$ and (ii) for small parameters $(\epsilon_i^2,\ \epsilon_a^2)$ leads to:
\begin{equation}
	\delta_z(\omega) = 
	\delta_{z,m}\left(1+\frac{1}{\omega^2}\right)
...
\begin{equation}
	\delta_x^2(\omega) = \epsilon_a^2\omega^2
