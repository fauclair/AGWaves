...
Each dispersion relation induces a relation between the non-dimensional wave-numbers $(\delta_x,\ \delta_z)$ and the non-dimensional pulsation $\omega$. For vanishing $\epsilon_i$ and $\epsilon_a$ parameters, the inner dispersion surface corresponds to the inner dispersion relations \ref{EqFullDisperai} for a pure imaginary vertical wave-number $\delta_z=i\delta_{z,i}$ and the y-axis showing $delta_{z,i}$. The inner (polychromatic) dispersion surface for pure-imaginary wave-number is a vertical plane (figure \oldref{FigFullHomogeneous}.a). When $\delta_x$ and $\delta_{z,i}$ are both positive the equation of the inner dispersion surface is given by $\delta_x-\delta_{z,i} =0$ in agreement with the conclusions reached in Subsection \ref{subsubsectioniR}. For an homogeneous, incompressible ocean, no wave can propagate both in the horizontal and vertical directions and the only waves which can propagate over the horizontal are vertically vanishing waves which only present an interest for a bounded ocean and should consequently be studies in the last section \ref{SectionAnalyticalSol}.
...
\subsection{Branches of the inner dispersion surface}
...
As already noted in the previous subsection, these later (red and yellow) surfaces cannot be distinguished from the upper and lower branches of the inner surface. These two branches are well-separated and the folded branch in between is the intermediate branch for pure-imaginary vertical wave-numbers.  This surface is (approximately) restricted to the region of phase-space $\delta_{z,i}\leq\delta_x$ as prognosticated in the previous section by requiring $R^2\leq1/4$. The upper and lower parts of the intermediate branch cannot be distinguished: the $\omega^2=\omega_a^2$ comes close to the upper part of the branch for small $\delta_x$ whereas the $\omega^2=\omega_i^2$ surface is close to the lower part for large $\delta_x$. This confirms that waves lying on the upper branch or on the upper part of the middle-range branch of the inner dispersion surface are compressibility waves whereas waves lying on the lower branch or on the lower part of the middle-range branch are gravity waves. The former (latter) are all the more "modified" by gravity (compressibility) that their pulsation decreases (respectively increases).\\
