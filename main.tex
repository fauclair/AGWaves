...
Surface waves can propagate but as long (mode-0) MIM solutio. 
Propagation in an homogeneous ocean can be studied with the present model by setting to zero the stratification parameter $\epsilon_i$ together with the last (advective) term on the right-hand-side of the inner dispersion relation \ref{EqFullDispera}: $-(\epsilon_i^2+\epsilon_a^2)^2/4$. The \textit{barotropic mode} $(n=0)$ disappears and the inner dispersion surface does not intersect the boundary dispersion surface in the vicinity of the origin: the horizontal and vertical wave-numbers of the barotropic MSW must simultaneously vanish. In an homogeneous ocean, the evanescent surface wave solution MSW is valid till the origin where $\delta_z$, $\delta_x$ and $\omega$ all vanish. \\
...
\begin{subequations}
	...
	\end{alignat}
\end{subequations}
For a vanishing horizontal wave-number, these parametric relations show that the pulsation vanishes too but the vertical wave-number is not exactly equal to $\pi$ but to $\pi+\epsilon_i^2/\pi$ and $\delta_z$ varies with the square of the stratification parameter $(\epsilon_i)$.\\
...
\begin{subequations}
	...
	\begin{alignat}{2}	
		...
		\\[3mm]
		\label{ParamSMAM2}
		&\delta_{z}(\omega)=\delta_{z,smam}(\omega) &&= \delta_{z,m}
		+(\epsilon_a^2-\epsilon_i^2)
		\left(-\frac{1}{2\delta_{z,m}}
		+\frac{1}{\omega^2}\left(
		\delta_{z,m}-\frac{1}{\delta_{z,m}}\right)
		\right)
		+\mathrm{O}	(\omega^{-3},\epsilon_i^4,\epsilon_a^4)
	\end{alignat}
\end{subequations}
