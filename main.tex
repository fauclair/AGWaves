...
The limit of LMSW for homogeneous and incompressible ocean (the $\sqrt{gH}$ long wave) is often presented as a barotropic (i.e. depth-independent) surface wave. This is however not exactly the case since $\delta_z=i\delta_x$, meaning that this long wave is exactly depth-independent only for a vanishing horizontal wave-number. Waves propagating with vanishing vertical wave-number ($\delta_z=0$) have however already been encountered in subsection \ref{SubSectionDisp} where they have been shown to satisfy the dispersion relations \ref{EqDispInnerkz0} and \ref{EqDispBoundkz0}. In non-dimensional form, these relations can be rewritten:
\begin{subequations}
